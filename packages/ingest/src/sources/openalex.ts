...
/** Curated terms (Plan §M4). `skill` is matched against the catalogue by label/alias. */
export const RADAR_TERMS: Array<{ term: string; search: string; skill: string; note: string }> = [
  { term: "Battery management systems", search: "battery management system", skill: "battery management",
    note: "EV supply chain around Chakan and Chh. Sambhajinagar; few MH ITI syllabi cover BMS yet" },
  { term: "EV charging infrastructure", search: "electric vehicle charging station", skill: "ev charging",
    note: "State EV policy 2025 targets charging corridors; electricians need EVSE installation skills" },
  { term: "Rooftop solar PV", search: "rooftop solar photovoltaic", skill: "solar pv",
    note: "PM Surya Ghar drives rooftop installs; Suryamitra is the matching PMKVY role" },
  { term: "Industrial IoT", search: "industrial internet of things", skill: "iot",
    note: "Predictive maintenance on Pune/Nashik shop floors" },
  { term: "Collaborative robots", search: "collaborative robot manufacturing", skill: "robot",
    note: "Cobot cells in auto-component MSMEs" },
  { term: "Cold chain logistics", search: "cold chain logistics", skill: "cold chain",
  ...
  { term: "Additive manufacturing", search: "additive manufacturing", skill: "3d printing",
    note: "Tooling and prototyping in capital goods" },
  { term: "Drones for agriculture", search: "agricultural drone", skill: "drone",
    note: "Kisan drone scheme; DGT runs a Drone Technician trade" },
  { term: "Generative AI", search: "large language model", skill: "machine learning",
    ...
    async fetch() {
      ...
      for (const t of RADAR_TERMS) {
        ...
        const [skill] = await sql<{ id: string }[]>`
          select s.id from ks.skill s left join ks.skill_alias a on a.skill_id = s.id
          where s.label_en ilike ${"%" + t.skill + "%"} or a.alias ilike ${"%" + t.skill + "%"}
          order by length(s.label_en) limit 1`;
